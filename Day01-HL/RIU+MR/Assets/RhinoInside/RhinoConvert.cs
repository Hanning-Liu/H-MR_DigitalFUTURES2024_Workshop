...
static class RhinoConvert
{
    ...
    static public UnityEngine.Quaternion ToHost(this UnityEngine.Quaternion p)
    {
        // Convert the original quaternion to Euler angles in XYZ coordinate system
        Vector3 eulerXYZ = p.eulerAngles;
        // Reorder Euler angles to XZY coordinate system
        Vector3 eulerXZY = new Vector3(eulerXYZ.x, eulerXYZ.z, eulerXYZ.y);

        var result = UnityEngine.Quaternion.Euler(eulerXZY);
        return result;
    }
